...
public function cadastrarFilme(FilmeDTO $FilmeDTO){
    try{
    $sql = "INSERT INTO  filme  (nome_filme,dt_de_lancamento_filme,duracao_filme, sinopse_filme, fk_categoria_filme_id_categoria_filme, classificacao_filme,capa_filme,trailer_filme, fk_usuario_id_usuario, fk_canal_filme_id_canal_filme) VALUES (?,?,?,?,?,?,?,?,?,? )";
    ...
    $stmt->bindValue(5, $FilmeDTO->getFk_categoria_filme_id_categoria_filme());
    ...
    $stmt->bindValue(10, $FilmeDTO->getFk_canal_filme_id_canal_filme());
...
public function alterarFilme(FilmeDTO $FilmeDTO) {
    try {
        $sql = "UPDATE filme SET nome_filme=?, sinopse_filme=?, categoria_filme=?, classificacao_filme=?, capa_filme=?, trailer_filme = ?, fk_usuario_id_usuario = ?, fk_categoria_filme_id_categoria_filme = ?, fk_canal_filme_id_canal_filme = ? WHERE id_filme=?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(1, $FilmeDTO->getNome_filme());
        $stmt->bindValue(2, $FilmeDTO->getDt_de_lancamento_filme());
        $stmt->bindValue(3, $FilmeDTO->getDuracao_filme());
    ...
    try{
        ...
        if($stmt->rowCount() > 0){
            while ($filmeFetch = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $FilmeDTO->setFk_categoria_filme_id_categoria_filme($filmeFetch['categoria_filme']);
                $FilmeDTO->setFk_usuario_id_usuario($filmeFetch['nome_usu']);
